...
async def voice_session(on_status):
    ...
    spk = sd.RawOutputStream(samplerate=OUT_RATE, channels=1, dtype="int16", blocksize=CHUNK)
    spk.start()
    try:
                        ...
                        for part in sc.model_turn.parts:
                            if getattr(part, "inline_data", None):
                                if isinstance(part.inline_data.data, bytes):
                                    ...
                                    if not turn_saved:
                                        try:
                                            save_item("[voice] conversation",
                                                      "voice agent analysis", None,
...
class AuctionEyeApp:
    ...
    def _build_ui(self):
        ...
        # Teal bar shown when voice is active
        ...
        # VOICE STATUS card (hidden until voice ON)
        ...
        def stat_row(parent, label, var, color):
            ...
            tk.Label(r, textvariable=var, bg=BG3, fg=color, font=("Courier",12,"bold")).pack(side="right")
    ...
    def _update_ui(self, item_name, price_summary, avg_price, offer, advice):
        ...
        tk.Label(row, text="◆", bg=BG3, fg=dot, font=("Courier",8)).pack(side="left", padx=(0,6))
        tk.Label(row, text=item_name.title(), bg=BG3, fg=WHITE, font=("Courier",9)).pack(side="left")
        if offer:
            tk.Label(row, text=f"${offer:.0f}", bg=BG3, fg=GOLD, font=("Courier",9,"bold")).pack(side="right")
    ...
    def _set_voice_ui(self, on: bool):
        if on:
            ...
            self.history_placeholder.pack_forget()
            row = tk.Frame(self.history_frame, bg=BG3); row.pack(fill="x", pady=2)
            tk.Label(row, text="◆", bg=BG3, fg=TEAL, font=("Courier",8)).pack(side="left", padx=(0,6))
            tk.Label(row, text="[voice conversation]", bg=BG3, fg=TEAL, font=("Courier",9)).pack(side="left")
